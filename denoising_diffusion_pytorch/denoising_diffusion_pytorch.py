...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def forward_and_back(self, img, t=250, use_q_posterior: bool = False):
        device = self.betas.device
        ...
        if use_q_posterior:
            ...
            for i in tqdm(
                ...
                img = self.p_sample(
                    img, torch.full((b,), t, device=device, dtype=torch.long)
                )

        intermediary = img.clone()
        ...
        # img = unnormalize_to_zero_to_one(img)
        # intermediary = unnormalize_to_zero_to_one(intermediary)
        return img, intermediary

    @torch.no_grad()
    def test(self, img, t=250):
        device = self.betas.device
...
class Dataset(data.Dataset):
    def __init__(
        ...
        self.paths = [x for x in self.paths if not str(x.name).startswith(".")]
        ...
        self.transform = A.Compose(
            transforms=[
                ...
                # A.PadIfNeeded(image_size, image_size, border_mode=0, value=0),
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.ds = Dataset(
            folder, image_size, augment_horizontal_flip=augment_horizontal_flip
        )
        self.dl = cycle(
            data.DataLoader(
                self.ds,
